-- Agora Tab Registry [UNIVERSELLE]
...
if not game then
	warn("[AGORA REGISTRY] game est nil")
	return function() end
end

return function(registryPage, shared)
local Players = shared.Players or game:GetService("Players")
local RunService = shared.RunService or game:GetService("RunService")
local UserInputService = shared.UserInputService or game:GetService("UserInputService")
local TextChatService = shared.TextChatService or game:GetService("TextChatService")
local Workspace = shared.Workspace or game:GetService("Workspace")
local ReplicatedStorage = shared.ReplicatedStorage or game:GetService("ReplicatedStorage")
local TweenService = shared.TweenService or game:GetService("TweenService")
local HttpService = shared.HttpService or game:GetService("HttpService")
local SoundService = shared.SoundService or game:GetService("SoundService")
local LocalPlayer = shared.LocalPlayer or Players.LocalPlayer
local Camera = shared.Camera or Workspace.CurrentCamera

local createCorner = shared.createCorner
local createStroke = shared.createStroke
local tween = shared.tween
local createButton = shared.createButton
local createSwitch = shared.createSwitch
local playSound = shared.playSound
local httpGet = shared.httpGet
local httpPost = shared.httpPost
local updateCharacter = shared.updateCharacter
local getDeviceType = shared.getDeviceType
local screenGui = shared.screenGui
local mainFrame = shared.mainFrame

_G._resolveCanChat = _G._resolveCanChat or function() end
_G._chatSeenPlayers = _G._chatSeenPlayers or {}
...
playersScroll:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
