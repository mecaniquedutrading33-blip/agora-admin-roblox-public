-- Agora Registry [UNIVERSELLE] - Panel de recherche Roblox externe
-- Se charge via loadstring(game:HttpGet("URL"))()

if not game then
	local ok, envGame = pcall(function() return getfenv().game end)
	if ok and envGame then game = envGame end
end
if not game then warn("[AGORA REGISTRY] game est nil") return end
...
local function playSound(id, vol)
	...
	pcall(function()
		...
		task.delay(3.2, function() pcall(function() s:Destroy() end) end)
	end)
end
...
if not _G._resolveCanChat then
		...
		task.spawn(function()
			...
			if uid and LocalPlayer then
				...
				if rf and rf:IsA("RemoteFunction") then
					...
					if ok and r ~= nil then result, src = r, "CanTalkWithMe" end
				end
			end
			if result == nil and uid then
				local ok, r = pcall(function() return game:GetService("TextChatService"):CanUserChatAsync(uid) end)
...
screenGui.Name = "AgoraRegistryPanel"
...
topBarTitle.Text = "Agora Hub  Registre"
...
closeBtn.MouseButton1Click:Connect(function() playSound(6042053626,0.22) pcall(function() screenGui:Destroy() end) end)
...
playersScroll.CanvasSize = UDim2.new(0, 0, 0, 2000)
...
extraScroll.CanvasSize = UDim2.new(0, 0, 0, 2000)
