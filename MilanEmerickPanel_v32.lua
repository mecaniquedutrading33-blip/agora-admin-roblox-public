...
local function canWalkBetween(a, b, params)
	...
	local maxSlope = math.tan(math.rad(50)) * 1.5  -- pente raisonnable pour le personnage
	local probeStep = 1.2
	local prevGroundY = a.Y
	for d = 0, dist, probeStep do
		local t = math.min(d / dist, 1)
		local pos = a + dir * (dist * t)
		local r = Workspace:Raycast(pos + Vector3.new(0, 30, 0), Vector3.new(0, -60, 0), params)
		if not r then return false end
		local gy = r.Position.Y
		local slope = math.abs(gy - prevGroundY) / probeStep
		if slope > maxSlope then return false end
		prevGroundY = gy
		-- espace pour tenir debout
		local headHeight = a.Y + 4.5
		local ceiling = Workspace:Raycast(Vector3.new(pos.X, math.max(headHeight, gy + 0.2), pos.Z), Vector3.new(0, 4, 0), params)
		if ceiling then return false end
		-- pas de trou trop profond devant
		local nextPos = pos + dir * probeStep
		local r2 = Workspace:Raycast(nextPos + Vector3.new(0, 30, 0), Vector3.new(0, -60, 0), params)
		if not r2 then return false end
		if r2.Position.Y < gy - 3 then return false end
	end
	return true
end
