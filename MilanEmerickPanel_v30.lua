...
local function createPlayerEntry(plr)
	...
	local function showInventoryGui()
		...
		if target:FindFirstChildOfClass("Humanoid") then
			for _, item in ipairs(target:GetChildren()) do
				if item:IsA("Tool") then table.insert(items, "(EQ) " .. item.Name) end
			end
		end
	...
	local function stealFirstTool()
		...
		local toSteal
		for _, item in ipairs(target:GetChildren()) do
			if item:IsA("Tool") then toSteal = item break end
		end
		if not toSteal then
		...
		if not toSteal then
			notify("Backpack de " .. plr.Name .. " vide", 2)
			return
		end
		local clone = toSteal:Clone()
	...
	stealBtn.Size = UDim2.new(0, 58, 0, 24)
	stealBtn.Position = UDim2.new(1, -68, 0, 110)
	stealBtn.BackgroundColor3 = Color3.fromRGB(160, 60, 60)
	stealBtn.Text = "Voler"
	...
	createCorner(stealBtn, 6)
	stealBtn.MouseButton1Click:Connect(stealFirstTool)
