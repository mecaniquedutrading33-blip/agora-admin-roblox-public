...
local function createPlayerEntry(plr)
	...
	invBtn.MouseButton1Click:Connect(function()
		if inventoryOpen then
			...
			invFrame.Position = UDim2.new(0, math.max(10, card.AbsolutePosition.X - 270), 0, card.AbsolutePosition.Y)
...
local speedLockConnection = RunService.RenderStepped:Connect(function()
	if humanoid and humanoid.WalkSpeed ~= walkSpeedState.value then
		humanoid.WalkSpeed = walkSpeedState.value
	end
	if humanoid and humanoid.JumpPower ~= jumpPowerState.value then
...
platformLabel.Position = UDim2.new(0, 8, 0, 286)
