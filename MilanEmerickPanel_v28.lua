...
local function shutdownPanel()
	...
	-- Destroy active tools created by panel (eleven tool etc)
	local backpack = LocalPlayer:FindFirstChild("Backpack")
	if backpack then
		for _, t in ipairs(backpack:GetChildren()) do
			if t.Name == "AutoClickTool" or t.Name == "ElevenTool" then
				t:Destroy()
			end
		end
	end
...
local function createPlayerEntry(plr)
	...
	copySkinBtn.MouseButton1Click:Connect(function()
		local ok, err = pcall(function()
			...
			if not srcChar or not myHumanoid then return end
			local srcHumanoid = srcChar:FindFirstChildOfClass("Humanoid")
			local desc = nil
			if srcHumanoid then
				desc = srcHumanoid:GetAppliedDescription()
			end
			if not desc then
				desc = Instance.new("HumanoidDescription")
				local humDesc = srcHumanoid or srcChar:FindFirstChildOfClass("Humanoid")
				if humDesc then
					desc.Head = humDesc.HeadColor and humDesc.HeadColor.Name or ""
				end
			end
			myHumanoid:ApplyDescriptionReset(desc)
			showNotification("Skin copié de " .. plr.Name, Color3.fromRGB(120, 200, 100))
		end)
		if not ok then
			showNotification("Skin inaccessible en local", Color3.fromRGB(255, 80, 80))
		end
	end)
	...
	local function refreshInventory()
		...
		local scroll = invFrame:FindFirstChild("ScrollingFrame")
		if not scroll then return end
		for _, c in ipairs(scroll:GetChildren()) do
			if c:IsA("TextButton") or c:IsA("TextLabel") then c:Destroy() end
		end
		local items = {}
		local targetBackpack = plr:FindFirstChild("Backpack")
		local targetChar = plr.Character
		...
		if #items == 0 then
			...
			for i, tool in ipairs(items) do
				...
				row.MouseButton1Click:Connect(function()
					...
					if clone then clone.Parent = LocalPlayer.Backpack end
				end)
			end
		end
		scroll.CanvasSize = UDim2.new(0, 0, 0, math.max(#items * 32, scroll.AbsoluteSize.Y))
	...
	invBtn.MouseButton1Click:Connect(function()
		if inventoryOpen then
			...
			if invFrame then closeInventory() end
			inventoryOpen = true
			invBtn.Text = "Fermer"
			...
			invFrame.Position = UDim2.new(0.5, -130, 0.5, -150)
			...
			invTitle.Size = UDim2.new(1, -34, 0, 24)
			...
			table.insert(invConns, plr.Backpack.ChildAdded:Connect(function() if inventoryOpen then refreshInventory() end end))
			table.insert(invConns, plr.Backpack.ChildRemoved:Connect(function() if inventoryOpen then refreshInventory() end end))
			table.insert(invConns, plr.CharacterAdded:Connect(function() if inventoryOpen then task.delay(0.5, refreshInventory) end end))
...
-- Rafraîchit l'ESP toutes les 60s pour capter les changements de skin
 task.spawn(function()
	while true do
		task.wait(60)
		if espState.enabled or globalESPEnabled then
...
local function startAutoClickEngine()
	if not autoClickState.targetGui then return end
	stopAutoClickEngine()
	...
	local function loop()
		while autoClickState.active and autoClickState.activeThread == threadId and autoClickState.targetGui and autoClickState.targetGui.Parent do
			local ap = autoClickState.targetGui.AbsolutePosition
			local as = autoClickState.targetGui.AbsoluteSize
			local cx = ap.X + as.X / 2
			local cy = ap.Y + as.Y / 2
			fireClickAtPos(cx, cy)
			task.wait(interval)
		end
	end
...
local function removeClickTarget()
	stopAutoClickEngine()
	for _, c in ipairs(autoClickState.targetConnections) do
		pcall(function() c:Disconnect() end)
	end
	autoClickState.targetConnections = {}
	if autoClickState.targetGui then
		autoClickState.targetGui:Destroy()
		autoClickState.targetGui = nil
	end
	setAutoClickSave()
end
...
local function createClickTarget()
	...
	targetGui.Name = "AutoClickTargetGui"
	targetGui.ResetOnSpawn = false
	targetGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	targetGui.Parent = LocalPlayer.PlayerGui
	autoClickState.targetGui = targetGui

	local frame = Instance.new("Frame")
	frame.Name = "ClickZone"
	...
	-- Drag + clamp à l'écran
	local dragging, dragStart, startPos
	local function clampFrame()
		local scr = targetGui.AbsoluteSize
		local abs = frame.AbsoluteSize
		local x = math.clamp(frame.AbsolutePosition.X, 0, math.max(0, scr.X - abs.X))
		local y = math.clamp(frame.AbsolutePosition.Y, 0, math.max(0, scr.Y - abs.Y))
		frame.Position = UDim2.new(0, x, 0, y)
	end

	local downConn = frame.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			dragging = true
			dragStart = input.Position
	...
	table.insert(autoClickState.targetConnections, downConn)
	table.insert(autoClickState.targetConnections, changeConn)
	table.insert(autoClickState.targetConnections, upConn)
...
infoLabel.Text = "Le carré rouge = zone de clic. Drag pour déplacer. Start = clic auto au centre."
...
startStopBtn.MouseButton1Click:Connect(function()
	if autoClickState.active then
		...
		if not autoClickState.targetGui then createClickTarget() end
