...
-- ===== ANIM ENTREE "cristallisation" : panel apparait avec zoom + rotation + glow =====
local entrySplash = Instance.new("Frame")
entrySplash.Name = "EntrySplash"
entrySplash.Size = UDim2.new(1, 0, 1, 0)
entrySplash.BackgroundColor3 = Color3.fromRGB(120, 80, 255)
entrySplash.BackgroundTransparency = 1
entrySplash.BorderSizePixel = 0
entrySplash.ZIndex = 50
entrySplash.Parent = mainFrame

local entryRing = Instance.new("Frame")
entryRing.Size = UDim2.new(0, 200, 0, 200)
entryRing.Position = UDim2.new(0.5, -100, 0.5, -100)
entryRing.BackgroundTransparency = 1
entryRing.BorderSizePixel = 4
entryRing.BorderColor3 = Color3.fromRGB(150, 200, 255)
entryRing.ZIndex = 51
entryRing.Parent = entrySplash
createCorner(entryRing, 100)

local entryDot = Instance.new("Frame")
entryDot.Size = UDim2.new(0, 0, 0, 0)
entryDot.Position = UDim2.new(0.5, 0, 0.5, 0)
entryDot.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
entryDot.BorderSizePixel = 0
entryDot.ZIndex = 52
entryDot.AnchorPoint = Vector2.new(0.5, 0.5)
entryDot.Parent = entrySplash
createCorner(entryDot, 100)

local entryText = Instance.new("TextLabel")
entryText.Size = UDim2.new(0.8, 0, 0, 30)
entryText.Position = UDim2.new(0.1, 0, 0.5, 20)
entryText.BackgroundTransparency = 1
entryText.Text = "Milan x Emerick"
entryText.Font = Enum.Font.GothamBlack
entryText.TextSize = 18
entryText.TextColor3 = Color3.fromRGB(255, 255, 255)
entryText.TextTransparency = 1
entryText.ZIndex = 52
entryText.Parent = entrySplash

-- Lance l'anim avec un petit delai pour qu'elle demarre apres le boot
task.defer(function()
	task.wait(0.3)
	-- Flash blanc rapide (effet "cristallisation") par-dessus le panel deja visible
	tween(entrySplash, {BackgroundTransparency = 0.3}, 0.15)
	tween(entryDot, {Size = UDim2.new(0, 600, 0, 600), BackgroundTransparency = 0.5}, 0.4)
	tween(entryRing, {Size = UDim2.new(0, 500, 0, 500), BackgroundTransparency = 1, BorderTransparency = 1}, 0.5)
	tween(entryText, {TextTransparency = 0.4}, 0.2)
	task.wait(0.4)
	-- Splash + text fade out
	tween(entrySplash, {BackgroundTransparency = 1}, 0.4)
	tween(entryText, {TextTransparency = 1}, 0.4)
	task.delay(0.5, function() if entrySplash and entrySplash.Parent then entrySplash:Destroy() end end)
end)
...
local function startAutoClickEngine()
	...
	task.spawn(function()
		while autoClickState.clickEnabled and autoClickState.activeThread == threadId do
			fireClickFixed(false)  -- boucle : PAS de VirtualInputManager (évite téléport)
			task.wait(interval)
		end
	end)
end
...
UserInputService.InputBegan:Connect(function(input, gpe)
	...
	if input.KeyCode == Enum.KeyCode.F10 then
		...
		if platformState.enabled then
			...
			updateCharacter()
			-- Détection intelligente : si on est assis dans un véhicule, on spawn sous les roues du véhicule
			-- sinon on spawn sous nos pieds
			local seatPart = humanoid and humanoid.SeatPart
			local target, targetSize
			if seatPart and seatPart:IsA("BasePart") then
				-- On est dans un véhicule : plateforme sous le châssis du véhicule, pas sous nos pieds
				...
				if not target then
					-- fallback : utiliser le siège lui-même
			...
			elseif character then
				-- À pied : spawn exactement sous nos pieds (utiliser HumanoidRootPart bas)
				if rootPart then
					-- Position des pieds = bas de la bounding box du character
					...
					if ok and cf and size then
						...
						platformState.y = rootPart.Position.Y - 3
					end
				else
					target = character:GetPivot().Position
					platformState.y = target.Y - 3
				end
			end
			if target then
				platformState.offset = 0
				platformState.part.CFrame = CFrame.new(target.X, platformState.y - 0.5, target.Z)
				-- Suivre la cible (voiture ou joueur) en X/Z
				platformState.followTarget = (seatPart and seatPart:IsA("BasePart") and (seatPart:FindFirstAncestorOfClass("Model") or seatPart.Parent)) or character
			end
		else
			if platformState.part then
				platformState.part:Destroy()
				platformState.part = nil
				platformState.followTarget = nil
			end
		end
	end
end)
...
RunService.Stepped:Connect(function(_, dt)
	...
	if platformState.enabled and platformState.part then
		updateCharacter()
		-- Suit la cible d'origine (voiture ou character) en X/Z, ou recale sur rootPart
		local followT = platformState.followTarget
		local trackX, trackZ
		if followT and followT.Parent then
			if followT:IsA("Model") then
				local ok, cf = pcall(function() return followT:GetBoundingBox() end)
				if ok and cf then
					trackX, trackZ = cf.Position.X, cf.Position.Z
				end
			elseif followT:IsA("BasePart") then
				trackX, trackZ = followT.Position.X, followT.Position.Z
			end
		end
		if not trackX then
			local seatPart = humanoid and humanoid.SeatPart
			local target = (seatPart and seatPart:IsA("BasePart")) or rootPart
			if target then trackX, trackZ = target.Position.X, target.Position.Z end
		end
		if trackX and trackZ then
			...
			platformState.part.CFrame = CFrame.new(trackX, platformState.y + platformState.offset - 0.5, trackZ)
		end
	end
...
RunService.Heartbeat:Connect(function()
	...
	if protectionsState.antiTeleport then
		if isAntiTpSuspended() then
			-- Gel : ne pas toucher aux références pendant fly/noclip
		else
...
-- BOOT SAFE LAYER 3: absolute fallback (if everything above fails, force first tab visible after 3s)
local function safeBoot()
	...
	if not ok then
		warn("[MILAN] bootSequence crashed: " .. tostring(err))
		pcall(function() switchTab("Joueurs") end)
	end
end
safeBoot()
